...
@Controller
public class PurchaseSystemController {

    @Autowired
    private PurchaseOrderService purchaseOrderService;

    @RequestMapping("/showpurchasesystem")
    public String show(Model model, PageVO pageVO, Condition condition) {
        ...
        return "purchasing/procurementandstorage";
    }

    @RequestMapping("/deletePurchaseSystem")
    public String DeleteById(Integer[] cElt){
        for (int i = 0; i < cElt.length; i++) {
            purchaseOrderService.deleteDepoTheadById(cElt[i]);
        }
        return "redirect:/showpurchasesystem?current=1&pageSize=4";
    }
}
